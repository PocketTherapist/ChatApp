...
var MessageDB = [{"mid":"0",
                  ...
                  {"mid":"1",
                   ...
                   "nextTurn":"bot"},
                   {"mid":"2",
                   "message":"ご記入ありがとうございます。あなたにオススメの優良治療院は下記になります。どれか一つ選んで、予約に進んでください。",
                   "nextId":"userClinic",
                   "nextTurn":"bot"},
                   {"mid":"3",
...
var UserBackPainTypeTop3 = {"Top3":[{"tid":"0",
                           "typename":"筋・筋膜性腰痛",
                           "typelink":"/KinmakuSei"},
                           {"tid":"4",
                            "typename":"心因性",
...
var UserRecommendedClinicTop3 = {"Top3":[{"cid":"12",
                 "shopname":"まつむら鍼灸整骨院",
                 "postalcode":"662-0913",
                 "address":"兵庫県西宮市染殿町3-13",
                 "tel":"0798-38-8455",
...
ReceptionBot.prototype.NextAction = function(nextId){
   ...
   console.log("NextID:"+this.nextId+",LastID:"+this.lastId);
   if(this.nextId.indexOf("message") !== -1){
      var temp = this.nextId.split("Id");
      this.showMessage(Number(temp[1]));
      console.log(temp[0] + "Id" + temp[1]);
   }else if(this.nextId.indexOf("question") !== -1){
      var temp = this.nextId.split("Id");
      ...
      this.showUserClinic();
   }else if(this.nextId.indexOf("back") !== -1){
      this.removeNowComment();
   }else{
      console.error("nextが間違っています。next:"+this.nextId);
   }
}
